...
const EncontreForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.cedula || formData.cedula.length !== 11) {
      newErrors.cedula = "Ingresa una cédula válida (ej: 5.426.847-8)";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulación de envío
    setTimeout(() => {
      alert("¡Cédula registrada con éxito!");
      setIsSubmitting(false);
      
      // Opcional: resetear formulario
      // setFormData({ cedula: '', nombre: '', email: '', lugar: '' });
      // setErrors({});
    }, 1500);
  };
  ...
  return (
      <div className="bg-white rounded-b-2xl p-8 border border-gray-100">
        <h2 className="text-3xl font-bold text-gray-800 mb-2">Reportar cédula</h2>
        <p className="text-gray-600 mb-8">Completa los datos de la cédula encontrada</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:border-transparent outline-none transition-all ${
                errors.cedula ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-teal-500'
              }`}
            />
            {errors.cedula && (
              <p className="text-red-500 text-sm mt-1">{errors.cedula}</p>
            )}
            <p className="text-xs text-gray-500 mt-1">Solo números • Formato: 5.426.847-8</p>
          </div>

          {/* Nombre visible */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Nombre visible en el plástico <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              ...
              className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:border-transparent outline-none transition-all ${
                errors.nombre ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-teal-500'
              }`}
            />
            {errors.nombre && (
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Lugar del hallazgo <span className="text-gray-400">(opcional)</span>
            </label>
            <textarea
          ...
          >
            {isSubmitting ? 'Registrando...' : 'Registrar cédula encontrada'}
          </button>
        </form>
      </div>
  );
